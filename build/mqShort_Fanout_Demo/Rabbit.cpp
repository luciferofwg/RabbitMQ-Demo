    ...
    const std::string &strBindKey)
{
    ...
    m_vectQueuename.push_back(strQueuename);
    m_vectBindKey.push_back(strBindKey);
...
int CRabbitmqClient::InitMQ(
    ...
    const std::string &strExchange,
    const std::string &strExchangeType,
    const std::vector<std::string> &vectQueuename,
    const std::vector<std::string> &vectBindKey
)
{
    if (strHostname.empty() ||
        strUser.empty() ||
        ...
        strExchange.empty() ||
        strExchangeType.empty() ||
        vectQueuename.size() != vectBindKey.size() ||
        iPort < 0)
    {
        printf("初始化MQ失败，错误原因：参数不合法！\n");
        return -1;
    ...
    m_strExchange = strExchange;
    m_strExchangeType = strExchangeType;
    m_vectQueuename = vectQueuename;
    m_vectBindKey = vectBindKey;
...
int CRabbitmqClient::publish(const string &strMessage)
{
    ...
    if (m_strExchangeType == "direct")
    {
        //定义队列对象
        CQueue      queue(m_vectQueuename[0], true);
        ...
        if (qmClient.queue_bind(queue, exChange, m_vectBindKey[0], strErr) < 0)
        {
            cout << strErr << endl;
            return -1;
        }
    }
    else if (m_strExchangeType == "fanout")
    {
        //建立连接
        if (qmClient.Connect(strErr) < 0)
        {
        ...
        for (size_t i = 0; i < m_vectQueuename.size(); i++)
        {
            //定义队列对象
            CQueue      queue(m_vectQueuename[i], true);
            //step2 声明一个队列
            if (qmClient.queue_declare(queue, strErr) < 0)
            {
                cout << strErr << endl;
            ...
            //step3 将交换机，队列和direct的路由规则绑定 ,direct方式是完全匹配,可以按照队列名称绑定
            if (qmClient.queue_bind(queue, exChange, m_vectBindKey[i], strErr) < 0)
            {
                cout << strErr << endl;
                return -1;
            }
